...
As reference for the simulation we used a filtered, downsized version of the data from the muscat paper.
For space reasons, we here provide the prepared muscat reference data, i.e. the product of:

```{r, eval=FALSE}
ref <- readRDS("../04_simulation2/data/ref/week13.SCE.processed.rds")
...
To produce a realistic simulation with different DE signals for different cell types, we simulate by chunks:
...
probs <- list( cluster=c(0.8,0.2),
               ...
               group=c(0.5,0.5) )
set.seed(123)
sim <- bplapply(list(c(0.2,0), c(0,0.2)), BPPARAM=MulticoreParam(2, RNGseed=1234), FUN=\(x){
...
sim3 <- simData(ref, nc=3200, ns=8, nk=1, probs=c(list(1),probs[-1]), force=TRUE,
               dd=TRUE, p_dd=c(0.85,0,0.15,0,0,0), paired=TRUE, rel_lfc=0.2)
...
saveRDS(sim, file="simulated1.SCE.rds")
...
saveRDS(de[which(de$logFC!=0),], file="simulated1.truth.rds")

b <- scuttle::sumCountsAcrossCells(sim, sim$sample_id)
saveRDS(b, file="simulated1.bulk.rds")
pb <- aggregateData(sim)
saveRDS(pb, file="simulated1.PB.rds")

sce <- scater::logNormCounts(sim)
...
saveRDS(rd, file="simulated1.dimRed.rds")

b <- readRDS("simulated1.bulk.rds")
...
saveRDS(d, "simulated1.bulk.DEA.rds")
```
...
We run the DEAs on 12 2vs2 subsets of the data:
...
We apply the various bbhw options:
...
save(ssres, st, aucs, file="simulated1.res.RData")
```
