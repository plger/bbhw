...
d <- d[grep("mean", d$variable),]

# add auc data:
aucs$variable <- "AUPRC"
aucs$value <- aucs$AUC
aucs <- aucs[!grepl("raw", aucs$method),]
aucs$method <- renameScores(gsub("padj\\.", "", aucs$method), addNp=TRUE)
i <- intersect(colnames(aucs), colnames(d))
d2 <- rbind(as.data.frame(d)[,i],aucs[!grepl("Excitatory", aucs$celltype) & aucs$method %in% d$method,i])
ds <- d[,c("celltype","Dataset")]
ds <- ds[!duplicated(ds),]
d <- merge(d2, ds)

d$variable <- factor(gsub("mean\\.","",d$variable), c("precision","recall","F1","AUPRC"))
...
wPrior <- ifelse(grepl("no prior",mo), "no prior", "using bulk prior")

cols <- setNames(list(viridis::magma(99), viridis::inferno(99), viridis::viridis(99), viridis::cividis(99)),
                 names(ml))
...
ov <- sort(rowMeans(m, na.rm=TRUE) + matrixStats::rowMedians(m, na.rm=TRUE))
ov <- rowAnnotation(df=data.frame(row.names=mo,overall=ov), col=list(overall=circlize::colorRamp2(breaks=range(ov), colors = c("grey90","black"))))

ra <- NULL
hl <- lapply(names(ml), FUN=function(x){
  m <- ml[[x]][mo,]
  if(x==rev(names(ml))[[1]]) ra <- ov
...
```{r}
load("../03_simulation1/sim.nbins.RData")
ths1 <- ths
load("../02_MSdata/MS.nbins.RData")
...
if(is.factor(ths$method)) ths$method <- droplevels(ths$method)
...
ths <- ths[grep("mean",ths$variable),]
ths$variable <- factor(gsub("mean\\.","",ths$variable), c("precision","recall","F1"))
ths$method2 <- relevel(as.factor(gsub("FDR\\.local", "No prior", ths$method2)), "No prior")

pb <- ggplot(ths, aes(method2, value, fill=nBins, group=method)) + 
...
pdf("Figure5_combinations.pdf", width=10, height=12)
