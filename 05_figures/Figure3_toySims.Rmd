...
a <- readRDS("../00_stressData/CRS_neuronal_DEAs.rds")
...
load("../01_toySim/GBH_sim_res.RData")
set.seed(1)
...
cols <- setNames(RColorBrewer::brewer.pal(length(unique(d$method)), "Dark2"), unique(d$method))
...
p4 <- ggplot(simNull, aes(as.factor(nsplits), FP, fill=method)) + geom_boxplot() + theme_bw() + scale_fill_manual(values=cols) +
