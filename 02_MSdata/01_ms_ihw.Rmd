...
The data comes from https://zenodo.org/records/8338963 ; here we use a pre-compiled pseudo-bulk object, filtered to the cell types that have a decent DEA signal in the full data.
...
assays(pb) <- assays(pb)[1:4]
saveRDS(pb, "pb_broad_filtered.rds")
```

For size reason, we share on github the filtered one, so skip to this:

```{r objPrep, eval=TRUE}
pb <- readRDS("pb_broad_filtered.rds")
...
saveRDS(b, file="ms_bulk.rds")

## Run DEA on the full-sized pseudobulk data:
...
saveRDS(ssres, file="ms_res.rds")
```

Run the bbhw variants:

```{r}
ssres <- readRDS("ms_res.rds")
set.seed(123)
...
sslfc <- bplapply(ssres, BPPARAM=MulticoreParam(6, RNGseed=1234, progress=TRUE), FUN=function(x){
  x <- x[,intersect(c("celltype", "gene", "cluster_id", "PValue", "logFC", "FDR"), colnames(x))]
...
## Add the local & global FDR without prior
set.seed(123)
